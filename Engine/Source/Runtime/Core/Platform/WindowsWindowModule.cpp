...
struct UpdateKeyStateOnMain
{
    KeyCode KeyCode;
    bool    IsDown;

    static void Run(UpdateKeyStateOnMain* arg)
    {
        InputModule& inputModule = InputModule::Get();
        inputModule.OnKeyEvent(arg->KeyCode, arg->IsDown);
    }
};
...
static LRESULT CALLBACK OmniWindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_RBUTTONDOWN:
    {
        ...
        DispatchWorkItem& dispatchWork =
            DispatchWorkItem::CreateWithFunctor(std::move(updateMouseOnMain), MemoryKind::CacheLine, true);
    ...
    case WM_KEYUP:
    {
        // printf("WM_KEYDOWN/UP: code:0x%x, pressed:%d\n", (u32)wParam, (lParam & (1 << 29)) != 0);
        bool pressed = uMsg == WM_KEYDOWN;

        if ((!pressed) || (lParam & (1 << 29)) ==
                              0) // WM_KEYDOWN may be sent repeatedly, only the one with transition is valid for us
        {
            UpdateKeyStateOnMain updateKeyStateOnMain;
            updateKeyStateOnMain.KeyCode = (KeyCode)wParam;
            updateKeyStateOnMain.IsDown = pressed;

            DispatchWorkItem& dispatchWork =
                DispatchWorkItem::CreateWithFunctor(std::move(updateKeyStateOnMain), MemoryKind::CacheLine, true);
            ConcurrencyModule::Get().EnqueueWork(dispatchWork, QueueKind::Main);
        }
        break;
    }
...
{ // can be called on any thread, for it will send a request to Window's internal message queue
    ...
    CheckWinAPI(SetWindowPos(self->mWindow,
        HWND_TOPMOST,
        rect.left,
        rect.top,
...
void WindowsWindowModulePrivate::RunUILoop(InitUIThreadArgs& initArgs)
{
    ...
    RECT rect = CalcNonClientWindowRect(0, 0, bfw, bfh);

    self->mWindow = CreateWindowEx(OmniMainWindowAdditionalStyle, // Optional window styles.
        ...
        CW_USEDEFAULT,
        rect.right - rect.left,
        rect.bottom - rect.top, // Size and position
        NULL,                   // Parent window
        NULL,                   // Menu
