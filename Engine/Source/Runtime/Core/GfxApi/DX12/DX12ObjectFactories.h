...
struct ID3D12GraphicsCommandList4CacheFactory final : public IObjectCacheFactory
{
    ...
    ID3D12GraphicsCommandList4CacheFactory(ID3D12Device* device, D3D12_COMMAND_LIST_TYPE cmdType)
        : mDevice(device)
        , mCmdType(cmdType)
    {
        mDevice->AddRef();
        CheckDX12(mDevice->CreateCommandAllocator(cmdType, IID_PPV_ARGS(&mDummyAllocator)));
    }
    ...
    void* CreateObject() override
    {
        ...
        auto ret = mDevice->CreateCommandList(0, mCmdType, mDummyAllocator, nullptr, IID_PPV_ARGS(&p));
        CheckDX12(ret);
        CheckDX12(p->Close());
        return p;
    }
...
private:
    ...
    D3D12_COMMAND_LIST_TYPE mCmdType;
};
...
struct ID3D12CommandAllocatorCacheFactory final : public IObjectCacheFactory
{
    ...
    ID3D12CommandAllocatorCacheFactory(ID3D12Device* device, D3D12_COMMAND_LIST_TYPE cmdType)
        : mDevice(device)
        , mCmdType(cmdType)
    {
        mDevice->AddRef();
    ...
    void* CreateObject() override
    {
        ...
        mDevice->CreateCommandAllocator(mCmdType, IID_PPV_ARGS(&p));
        return p;
    }
    void DestroyObject(void* obj) override
    {
